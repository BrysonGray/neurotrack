...
def decode_direct_vector_output(
    policy_output: torch.Tensor,
    stop_action_threshold: float = 0.5,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor | bool]:
    """Decode direct-vector policy output into bounded direction and stop decisions."""
    ...
    direction = bound_direction_magnitude(output_t[:, :3], max_norm=10.0)
