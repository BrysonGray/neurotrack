...
class DataGenerator:
    ...
    def generate_reward_mask(self, subtree: List, shape: Tuple[int, ...], width=3.0) -> torch.Tensor:
        ...
        reward_mask = self.renderer.draw_density(sections, shape, width=width, mask=False)
            ...
            try:
                ...
                for i, subtree in enumerate(subtrees):
                    ...
                    if img_tensor is not None:
                        ...
                        reward_mask = self.generate_reward_mask(shifted_subtree, cropped_image.shape[1:], width=float(config.width))
                        result = {
                            'image': cropped_image,
